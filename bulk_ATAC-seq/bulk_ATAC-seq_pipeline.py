...
def main(args):
	...
	if not args.skip_rpkm:
		try:
			...
			make_rpkm(args, rmdup_bam)
		except Exception as e:
			logging.error('Failed during RPKM-normalized track generation step.')
			print(repr(e), file=sys.stderr)
			sys.exit(1)
...
def process_args():
	parser = argparse.ArgumentParser(description='Pipeline for ATAC-seq to trim reads, align them to a reference genome, call peaks, and generate genome browser signal tracks.')
	...
	align_group.add_argument('-ref', '--reference', required=False, type=str, default='ref/male.hg19.fa', help='Path to reference genome [./ref/male.hg19.fa]')
	align_group.add_argument('--picard_mark_dup', required=False, type=str, default='etc/MarkDuplicates.jar', help='Path to picard MarkDuplicates.jar [./etc/MarkDuplicates.jar]')
	...
	qc_group.add_argument('--blacklist', required=False, type=str, default='etc/ENCODE.hg19.blacklist.bed', help='Path to blacklist BED file to ignore ENCODE high signal regions [./etc/ENCODE.hg19.blacklist.bed]')
	
	bedgraph_group = parser.add_argument_group('Bedgraph track arguments')
	...
	rpkm_group = parser.add_argument_group('Normalized signal track arguments')
	rpkm_group.add_argument('--genomeSize', required=False, type=int, default=2864785220, help='Effective genome size for RPKM normalization (default: 2864785220 for hg19')
	...
	skip_group.add_argument('--skip_rpkm', required=False, action='store_true', default=False, help='Skip making rpkm normalized browser track [OFF]')
